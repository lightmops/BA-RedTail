...
\label{sec_wandler}
- Licht wird mit Filtern in drei Farbanteile R, G, B aufgespalten
- Variante 1 mit Strahlenteiler Prisma auf drei verschiedene Wandler
- früher teildurchlässige Spiegel mit dichroitischen Schichten bedampft, heute fest verbundener Prismensatz
- Licht wird aufgespalten und nur R, G, B- Anteile zum jeweiligen Wandler durchgelassen (Abbildung)
- Problem: Filter sind nicht ideal (blaues licht trifft auch auf R oder G filter ) --> Farbkorrektur Filter vor jedem Wandler --> Abbildung Filterwirkung
- single Sensor kameras arbeiten mit aufgedampften Bayer Pattern 50\% blau und 25\% rot und 25\% grün --> Farbauflösung verringert. 
Schmidt 380-381

\section{Weißabgleich}
\label{sec_wb}
- unbuntabgleich auf bildweiß und bildschwarz
- Kamera muss an tages / kunstlicht angepasst werden, damit weiß in kamera aucch weiß erscheint
- kamera wird auf weiße fläche gerichtet, die von lichtart beleuchtet wird
- in der kamera werden R- und B-Kanäle so verstärkt, dass alle drei kanäle gleiche Signalpegel haben
- im Studio betrieb wird über vektorskop sichergestellt, dass Signale keine buntanteile mehr enthält
-abbildung weißabgleich 
schmidt - 414-415

Schritt 1: Objekt kadrieren (auf) 80\% der Bildschirmgröße
Schritt 2: Farbfilterrad einstellen auf farbtemperatur kunst /tageslicht
Schritt 3: Speicherplatz wählen auf A oder B speichern
Schritt 4: Kamera misst Bildfarbe und korrigiert sie zu weiß 

AV-Medientechnik s. 206

\section{RGB-Signal}
\label{sec_rgbsignal}
- das RGB Signal besteht aus den drei Farbwertsignalen E'r, E'g und E'b (elektrisches Signal mit Gamma Vorverzerrung)
--> drei Übertragungskanäle
- 100\% Signalpegel = 0,7V
- Synchrosignal auf seperaten Leitung oder auch auf allen drei Leitung mit Signalpegel = -0,3V
- hohe Übertragungsqulität --> hoher bedarf an bandbreite --> nur auf kurzen strecken im studio genutzt
schmidt S. 82-83

\section{Komponenten-Signal}
\label{sec_ycrcb1}
- Anforderung an Farbfernsehen: Farbsignal muss s/w kompatibel sein --> Leuchtdichtesignal für s/w-Fernsehen leicht ableitbar
- zweitens sollen Farbinfos mit auf den Kanal des monochronem Videosignal  übertragen werden .. Farbfernsehen soll keine zusaätzliche Bandbreite kosten
- dazu musste RGB Signal angepasst werden

\begin{eqnarray}\label{gl_tlci2}
	Y = 0,299 \cdot R + 0,587 \cdot G + 0,114 \cdot B\\
	Y = 0,2126 \cdot R + 0,7152 \cdot G + 0,0722 \cdot B
\end{eqnarray}

- Gleichung 7.1 für SD 7.2 für HD
- signale sind gamma vorentzerrte signale
- damit zum Beispiel bei einer Bandbreitenreduktion nur Farbanteile und keine Helligkeiten reduziert werden wird neben dem Y-Signal auch ein R-Y und B-Y Singal übertragen dabei werden absichtlich R und B gewählt, da G-Y nur geringe Pegel ergibt, da Y hauptsächlich durch den Grünanteil bestimmt ist
- Farbdifferenzsignale verschwinden bei Unbunten Bilder da R=G=B=Y --> R-Y=B-Y=0
- die Pegel von Farbdifferenzsignalen können den max. Wert von 0,7 mV bei vollaussteurung überschreiten --> für vereinheitlichung müssen diese reduziert werden
...
\label{sec_wmf}
- Oszilloskope für Videomessungen
- basiert auf Braunscher Röhre mit Elektronenstrahl, der einen Bildpunkt der Leuchtschicht des Bildschirms anregt
- Messspannung für Ausrichtung des Strahls zuständig
- im XY-Betreib werden zwei Spannungen dafür genutzt siehe bild
- %Bild einfügen
- moderne Geräte arbeiten intern digital und simulieren Oszilloskopanzeige
- misst jeweils ein Bildzeile
- bei Messung wird nur y-Achse genutzt und x-achse läuft einfach nach bestimmer zeit durch
- bei periodischen signal wird messung getriggert damit pegelwerte zur selben zeit mit selber phasenlänge immer übereinander liegen
- durch filter kann nur luminanzsignalanteil, Chrominanzanteil oder beides dargestellt werden
- mit Farbbalken (jede Kombination der drei Farbsignale einmal durch: z.b. 100/100 Farbbalken) können die Signale von Kameras überprüft werden

Schmidt S. 109-112

\section{Vektorskop}
\label{sec_vector}
- Farbton ist mit WFM-Monitor nicht sehr gut erkennbar, da phasenwinkel im hochfrequenten Bereich nicht gut einzuordnen ist
- Vektorskop werden auch im XY-Betrieb genutzt. Ausrichtung in x-achse gemäß der FBAS-Signalkomponente U und y-asche gemäß der komponente V

U und V werden durch eine andere Pegelreduzierung der Farbsignale des Komponentensignals
...
diese Änderung komm daher, dass beim FBAS-Signal die Farbsignale in modulierter Form mit dem Luminanzsignal überlagert werden schmidt s. 84
