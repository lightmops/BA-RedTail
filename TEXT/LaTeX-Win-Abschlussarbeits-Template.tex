...
Es gibt zwei verschiedene Fälle, in denen der Mensch die Gesichtsemfpindung Farbe verspürt: Entweder leuchtet ein Objekt von selbst und dessen Lichtspektrum gelangt ins Auge und wird dann im Zusammenspiel mit dem Gehirn ein Farbreiz $\phi(\lambda)$ erkannt. In diesem Fall nennt man das Objekt einen Selbstleuchter. Oder das Objekt wird von einer Lichtquelle beleuchtet und das reflektierte Lichtspektrum wird wahrgenommen. Das Objekt aborbiert und transmittiert bestimme spektrale Anteile des Licht und reflektiert den Rest (Kapitel \ref{chap_grundlagen}). Daher können nur spektralen Anteile des Lichts reflektiert werden, die auch im Spektrum der Lichtquelle vorhanden sind. Ein roter Apfel erscheint deswegen von blauem Licht beleuchtet unbunt, da kein roter Spektralanteil reflektiert werden kann.
In diesem zweiten Fall spricht man von dem Objekt als Körperfarben\footnote{\cite[103]{hentschel}}.\\     
Damit ein Mensch einen Farbreiz wahrnehmen kann, gibt es im Auge zwei Arten von lichtempfindlichen Rezeptoren in der Netzhaut, die für unsere Farbwahrnehmung verantwortlich sind: Zapfen und Stäbchen.\\
Die Stäbchen nehmen verschiedene Helligkeitseindrücke wahr, können aber keine Farben unterscheiden. Daher sind sie für das skotopische Sehen (Nachtsehen) von 3 x $10^{-6} \frac{cd}{m^{2}}$ bis 0,03$\frac{cd}{m^{2}}$ verantwortlich \footnote{\cite{doccheck sko}}.
...
Das menschliche Auge nimmt die sichtbaren Wellenlängen von 380nm bis 780nm jedoch unterschiedlich stark wahr. Grüne Gegenstände (555nm) wirken daher immer heller als blaue (485nm) oder rote (680nm). Dieser Sachverhalt wurde mit der V($\lambda$)-Kurve  aufgezeigt (Abbildung \ref{b_augespek}).
...
\end{figure}
\footnotetext{\cite6[153]{greule}}

Um eine Farbe entstehen zu lassen gibt es zwei Möglichkeiten:

Bei der additiven Farbmischung werden die Spektralanteile verschiedener Farbtöne als Mischfarbe erkannt. Dies kann auf unterschiedliche Weisen passieren. Entweder treffen zwei verschiedene Spektralanteile auf den gleichen Punkt auf der Netzhaut und lösen so einen Farbreiz aus (örtliche Nähe). Oder derselbe Punkt auf der Netzhaut wird von zwei verschiedenen Spektralanteilen mit einer Wechselfrequenz von $f\geq25Hz$ getroffen (zeitliche Nähe). Oder das Licht verschiedener Wellenlängen trifft auf unterschiedliche Teile der Netzhaut, die so dicht aneinander sind, dass das Auge daraus eine Farbe mischt.\\
