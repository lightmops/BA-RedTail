...
Bilder werden in \LaTeX\ am einfachsten mit includegraphics eingebunden und in eine figure-Umgebung eingebettet. Sie k�nnen mit optionalen Parametern skaliert werden; in Abbildung \ref{b_trommelmik} ist die Bildbreite auf das 0.8-fache des Satzspiegels skaliert.

Zu jeder Abbildung geh�rt eine nummerierte Bildunterschrift und ein Verweis im Text (Abb. \ref{b_trommelmik}). \LaTeX\ unterst�tzt das mit dem caption-Befehl. Mit den Befehlen label und ref werden symbolische Labels definiert und abgerufen. F�r Seitenverweise wird der Befehl pageref genutzt: Richtcharakteristiken sind i.Allg. frequenzabh�ngig (Abb. \ref{b_richtch} auf S. \pageref{b_richtch}). 

Mit label und ref kann man nicht nur auf Bilder oder Tabellen verweisen, sondern auf jede nummerierte Struktur, z.B. auf Gleichungen, Kapitel oder Unterkapitel: Zu Fu�noten siehe Abschnitt \ref{sec_fussnot}. 
...
Praktische Tipps f�r die Erstellung von Bildern: Aufl�sung 300 dpi, bei Fotos gen�gen u.U. 150 dpi. Bei Scans von Strichzeichnungen sollte die Aufl�sung bezogen auf die Druckgr��e mindestens 1200 dpi betragen (siehe Abb. \ref{b_richtch}). 
...
% bei langen Bildunterschriften kann der optionale Parameter des caption-Befehls so wie hier f�r die Kurzfassung im Abbildungsverzeichnis genutzt werden  
\end{figure}
%------------- BILD ENDE ---------------

F�r Formelsatz stellt \LaTeX\ die nummerierte Umgebung equation und die nicht-nummerierte Umgebung displaymath zur Verf�gung. Mit label und ref kann dann im Text Bezug auf die Gleichungen genommen werden (Gleichung \ref{gl_fourier}). 

\begin{equation}\label{gl_fourier}
S(f) = \int_{-\infty}^{\infty} s(t)e^{-j 2 \pi f t}dt
\end{equation}

Mathematik im Zeilenmodus sieht so aus $f_0 = \frac{1}{2\pi} \sqrt{\frac{s}{m}}$, w�hrend dieselbe Gleichung als abgesetzte Formel -- hier mit der displaymath-Umgebung -- so aussieht: 

\begin{displaymath}
f_0 = \frac{1}{2\pi} \sqrt{\frac{s}{m}} 
\end{displaymath}

F�r mehrzeilige Herleitungen oder Berechnungen benutzt man in \LaTeX\ die Umgebung eqnarray.

Einheiten innerhalb von Formeln werden -- wie auch Text -- grunds�tzlich steil (nicht-kursiv) gesetzt. Innerhalb der mathematischen Umgebung nimmt man daf�r eine mbox (make box); die Abst�nde werden mit Komma, Semikolon oder quad eingestellt:

\begin{displaymath}
f_0 = \frac{1}{2\pi} \sqrt{\frac{s}{m}} \quad \mbox{[Hz]}
% \, kleiner Abstand \; mittlerer Abstand \quad gro�er Abstand
\end{displaymath}

Gleiches gilt f�r Funktionsnamen (sin, cos, arctan, log, ...). F�r die meisten Funktionsnamen gibt es aber zur Vereinfachung entsprechende Befehle, sodass man nicht immer die mbox braucht.


\section{Unterkapitel mit zwei Zitaten}

Das w�rtliche Zitat wird durch Kursivschrift und Anf�hrungszeichen kenntlich gemacht, und nat�rlich kommt ein Quellenverweis dazu:

% \emph = Texthervorhebung durch Schriftumschaltung (emphasize), \medskip: vertikaler Abstand (\smallskip, \bigskip)
\medskip
\emph{\glqq Ebenso wie die Sinne sind in der Klanggestalt die geistig-ideellen Bereiche mit den physisch-materiellen verbunden, d.h. die s�ngerische Intention muss sich den Prinzipien der Klangwahrnehmung unterordnen.\grqq} \citep[111]{sowodniok}.
\medskip

Alternativ kann man ein Zitat auch in den laufenden Text einflechten, denn wie schon Sowodniok bemerkte, muss sich 
\emph{\glqq die s�ngerische Intention [...] den Prinzipien der Klangwahrnehmung unterordnen\grqq} \citep[111]{sowodniok}. 
Die Quellenverweise werden weiter unten erkl�rt.


\chapter{Ein anderes Kapitel}

\section{Unterkapitel mit Fu�note, Aufz�hlungen und Tabellen}\label{sec_fussnot}

Fu�noten sollte man sparsam und bewusst verwenden, erkl�rende Zus�tze und Quellenverweise m�glichst in den Text integrieren. Damit bleiben Fu�noten v.A. reserviert f�r wenige Erg�nzungen, die den Lesefluss st�ren w�rden, aber nicht weggelassen werden sollen\footnote{Und so sieht die Fu�note dann aus}.

F�r Aufz�hlungen stellt \LaTeX\ die beiden Umgebungen itemize und enumerate zur Verf�gung. So sieht eine itemize-Aufz�hlung aus:

\begin{itemize}\setlength{\itemsep}{0ex} % itemsep ist der Abstand zwischen den Punkten der Aufz�hlung
\item erster Punkt
\item zweiter Punkt
\end{itemize}

Und das ist eine enumerate-Aufz�hlung:
...
Aufz�hlungen k�nnen auch verschachtelt werden. Als Beispiel dient hier eine enumerate-Umgebung innerhalb einer enumerate-Umgebung:
...
Als n�chstes folgt ein Beispiel f�r eine einfache Tabelle. Wie auch die Bilder m�ssen die Tabellen stets Unterschrift und Nummer und zwingend einen Verweis im Text haben. In \LaTeX\ wird das wie bei den Abbildungen durch den caption-Befehl und das Befehlspaar label und ref gel�st (Tabelle \ref{t_buli}). F�r ein modernes Tabellenlayout wird das \LaTeX-booktabs-Paket benutzt (siehe dazu die Kommentare im Quelltext). Die mittlere Spalte ist hier auf feste Breite (6 cm) gesetzt, damit bei viel Text ein automatischer Umbruch erfolgen kann.
...
2 & Borussia M�nchengladbach & +14 & 29\\ \midrule
3 & FC Bayern M�nchen & +26 & 28 \\ \midrule
...
Tabelle \ref{t_buli2} zeigt eine Variante die ein kompakteres und eleganteres Ergebnis liefert, ohne vertikale Striche, daf�r mit eingef�rbten Zeilen.
...
2 & Borussia M�nchengladbach & +14 & 29\\
3 & FC Bayern M�nchen & +26 & 28\\
...
Quellenverweise werden mit Autorennamen und Jahr in runden Klammern gesetzt. Dazu wird hier das \LaTeX-natbib-Paket genutzt; der citep-Befehl erzeugt die Quellenangabe auf Basis der Eintr�ge im Literaturverzeichnis \citep{bluray}. Auf gleiche Weise lassen sich auch mehrere Quellen zusammenfassen \citep{dooley_streicher,stephenson}. 

Auf B�cher oder andere umfangreichere Quellen soll mit Seitenangabe verwiesen werden. Daf�r stellt der Befehle citep  einen optionalen Parameter zur Verf�gung. Und so sieht dann die vollst�ndige Quellenangabe aus \citep[116]{kuttruff}. 
...
Und noch eine \LaTeX-Spezialit�t zum Schluss: Durch die Einbindung von url- und hyperref-Paket im header werden die Quellenverweise im PDF-Dokument automatisch mit der jeweiligen Quelle im Literaturverzeichnis verlinkt, und bei Internetquellen werden die URLs anklickbar. Zudem werden die Verzeichnisse (Inhaltsverzeichnis, Abbildungs- und Tabellenverzeichnis) mit den jeweiligen Objekten verlinkt, und es werden Links zwischen jedem \emph{label} und  dazugeh�rigem \emph{ref} erzeugt, also z.B. zwischen Bildverweis im Text und dem Bild. Die Farben der Links k�nnen im header frei eingestellt werden. Im hier vorgeschlagenen Layout sind die URLs und die Quellenverweise Dunkelblau, die anderen Links sind nicht hervorgehoben (Schwarz). 


\chapter{Ergebnisse}

Der thematische Teil schlie�t mit einer klaren inhaltlichen, auf der Grundidee aufbauenden thematischen Zusammenfassung, insbesondere bezogen auf die in der Arbeit gewonnenen eigenen Erkenntnisse und deren m�gliche Auswirkungen auf Forschung und Wissenschaft. 

Ganz am Schluss, nach eventuellen Anh�ngen, nach Abbildungs- und evtl. ( \- )Ta-\ bellenverzeichnis, und nach dem Literaturverzeichnis, folgt die Eigenst{"a}ndigkeitserkl{"a}rung, die unterschrieben werden muss.


\appendix
\chapter{Material}

\section{Frageb�gen, Messprotokolle etc.}
In den Anh�ngen landen ggf. Listings, Frageb�gen, Datenbl�tter, Messprotokolle, Skizzen zu Versuchsaufbauten und �hnliches Material zur Arbeit. Im \LaTeX-Dokument leitet der Befehl appendix die Anh�nge ein.
